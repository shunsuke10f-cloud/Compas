...
import { DIAGNOSIS_STATEMENTS } from "@/lib/job-hunting/questions";
...
export default function DiagnosisPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      window.localStorage.setItem("compas.jobHunting.typeKey", result.primaryType);
      ...
      router.push(`/job-hunting/result/${result.primaryType}`);
    }
  }
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-8 px-6 py-16 sm:px-10">
      <div>
        ...
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
          全{DIAGNOSIS_STATEMENTS.length}問。それぞれの文章が自分にどれくらい当てはまるかを5段階で選んでください。
          （{answeredCount}/{DIAGNOSIS_STATEMENTS.length}）
        </p>
        <p className="mt-1 text-xs text-zinc-500">
          6つのタイプそれぞれについて複数の角度から質問し、回答内容をもとにスコアの内訳と根拠を診断結果画面に表示します。
        </p>
      </div>

      <div className="flex flex-col gap-3">
        {DIAGNOSIS_STATEMENTS.map((statement, index) => (
              ...
              {[1, 2, 3, 4, 5].map((value) => (
                ...
                >
                  <input
                    ...
                    onChange={() => setAnswers((prev) => ({ ...prev, [statement.id]: value }))}
                    className="h-4 w-4"
                  />
                  {value}
                </label>
